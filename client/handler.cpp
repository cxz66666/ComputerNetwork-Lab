...
void printHelp(){
    ...
    cout<<"Named Commands:\n";
    cout<<"  isConnect [ip] [port]    isConnect to the given ip:host, usage: isConnect 127.0.0.1 4444\n";
    cout<<"  disconnect             disconnect if already isConnect to server\n";
...
void handleConnect(vector<string>&commands){
    ...
    try{
        ...
        printf("[client] isConnect need a int port, but got a illegal port\n");
        return;
    }
    cs->Lock();
    ...
    //already connected
    if(cs->hasConnected()){
        printf("[client] already isConnect!\n");
        cs->unLock();
        return;
    }
    ...
    if(!cs->setIpAndHost(commands[1].c_str(),port)){
        cs->init();
        printf("[client] isConnect get a illegal ip:host\n");
        cs->unLock();
        return;
    }
    ...
    if(cs->connectServer()<0){
        printf("[client] isConnect to server error, sockFd is %d\n",cs->sockFd);
        cs->init();
        cs->unLock();
        return;
    }
